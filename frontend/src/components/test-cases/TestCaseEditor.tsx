    ...
    <div className="fixed inset-0 z-[9990] flex items-center justify-center bg-black/60 backdrop-blur-sm">
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: 'var(--border-default)' }}>
          <h3 className="text-sm font-mono font-semibold text-slate-200">Edit Test Case</h3>
        ...
        <div className="p-4 flex flex-col gap-4">
          {/* Type selector */}
          <div>
            <label className="text-xs font-mono text-slate-500 mb-1.5 block">Type</label>
                  ...
                  className={`px-3 py-1 rounded text-xs font-mono font-semibold uppercase transition-colors ${
                    type === t
                      ? t === 'normal' ? 'bg-green-500/20 text-green-400 border border-green-500/40'
                        : t === 'edge' ? 'bg-cyan-500/15 text-cyan-400 border border-cyan-500/35'
                        : 'bg-red-500/20 text-red-400 border border-red-500/40'
                      : 'bg-slate-800 text-slate-500 border border-slate-700 hover:border-zinc-600'
                  }`}
                >
                  {t}
                </button>
          ...
          {/* Inputs */}
          <div>
            <label className="text-xs font-mono text-slate-500 mb-1.5 block">Inputs</label>
            <div className="flex flex-col gap-2">
              {Object.entries(inputs).map(([key, val]) => (
                <div key={key} className="flex items-center gap-2">
                  <span className="text-xs font-mono text-cyan-400 w-16 flex-shrink-0">{key}</span>
                  <input
                    type="text"
                    value={String(val)}
                    onChange={(e) => setInputs((prev) => ({ ...prev, [key]: e.target.value }))}
                    className="flex-1 px-2 py-1 text-xs font-mono rounded bg-slate-800 border border-slate-700 text-slate-200 focus:outline-none focus:border-cyan-500/50"
                  />
                </div>
              ))}
            </div>
          ...
          {/* Expected */}
          <div>
            <label className="text-xs font-mono text-slate-500 mb-1.5 block">Expected Output</label>
            <input
              type="text"
              value={expected}
              onChange={(e) => setExpected(e.target.value)}
              className="w-full px-2 py-1.5 text-xs font-mono rounded bg-slate-800 border border-slate-700 text-slate-200 focus:outline-none focus:border-cyan-500/50"
            />
          </div>

          {/* Coverage hint */}
          <div>
            <label className="text-xs font-mono text-slate-500 mb-1.5 block">Coverage Note</label>
            <input
              type="text"
              value={coverageHint}
