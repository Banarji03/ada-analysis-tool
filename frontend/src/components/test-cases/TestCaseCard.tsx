    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-2 px-3 py-2">
          {/* Drag handle */}
          <span
            className="text-zinc-700 hover:text-slate-400 cursor-grab active:cursor-grabbing flex-shrink-0"
          ...
          <span className="text-xs font-mono text-slate-500">Test_{index + 1}:</span>
        ...
        <div className="px-3 pb-2 pl-8">
          ...
          <p className="text-xs font-mono text-slate-400">
            Expected =&gt; <span className="text-slate-200">{String(testCase.expected)}</span>
          </p>
          {testCase.coverageHint && (
            <p className="text-[10px] font-mono text-cyan-500/70 italic mt-1">
              {testCase.coverageHint}
            </p>
          )}
        ...
        >
          <div className="pb-2 border-t pt-2" style={{ borderColor: 'var(--border-default)' }}>
            <p className="text-[10px] font-mono text-slate-600 mb-1">Full Input Record:</p>
            {Object.entries(testCase.inputs).map(([k, v]) => (
              <p key={k} className="text-[10px] font-mono text-slate-500 pl-2">
        ...
        <div
          ...
          onClick={(e) => e.stopPropagation()}
        >
          <button className="flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-mono text-slate-500 hover:text-green-400 hover:bg-green-500/10 transition-colors">
            <Play size={9} /> Run
          </button>
          <button
          ...
          <button
            ...
            className={`flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-mono transition-colors ${showNote ? 'text-purple-400 bg-purple-500/10' : 'text-slate-500 hover:text-purple-400 hover:bg-purple-500/10'}`}
          >
        ...
        {showNote && (
          <div className="px-3 pb-2 border-t" style={{ borderColor: 'var(--border-default)' }} onClick={(e) => e.stopPropagation()}>
            <textarea
              ...
              rows={2}
              className="w-full mt-2 px-2 py-1.5 text-[10px] font-mono rounded bg-slate-800 border border-slate-700 text-slate-300 placeholder-zinc-600 focus:outline-none focus:border-purple-500/50 resize-none transition-colors"
            />
          </div>
        )}
      </div>
