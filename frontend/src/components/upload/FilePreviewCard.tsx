...
export const FilePreviewCard: React.FC<FilePreviewCardProps> = ({ file, index }) => {
  ...
  return (
    <div
      className="slide-up flex items-center gap-3 px-4 py-3 rounded-lg bg-slate-800/60 border border-slate-700/50 hover:border-zinc-600 transition-all"
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-slate-200 text-sm font-mono truncate">{file.name}</p>
      ...
      <div className="flex items-center gap-1">
        <button
          ...
          title="Replace file"
          className="p-1.5 rounded text-slate-500 hover:text-slate-300 hover:bg-zinc-700 transition-colors"
        >
          <RefreshCw size={13} />
        </button>
        <button
