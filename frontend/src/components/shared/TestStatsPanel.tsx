...
export const TestStatsPanel: React.FC = () => {
  ...
  const stats = [
    { label: 'Total', value: total, icon: <TestTube size={13} />, color: 'text-slate-400' },
    ...
    { label: 'Pending', value: pending, icon: <Clock size={13} />, color: 'text-slate-500' },
  ];
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-mono font-semibold text-slate-300">Test Statistics</span>
        <span className="ml-auto text-xs font-mono font-bold text-cyan-400">{passRate}% pass rate</span>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2">
        {stats.map((s) => (
          <div key={s.label} className="flex flex-col items-center gap-1 p-2 rounded-lg bg-slate-800/40">
            <span className={s.color}>{s.icon}</span>
            <span className={`text-sm font-mono font-bold ${s.color}`}>{s.value}</span>
            <span className="text-[9px] font-mono text-slate-600 uppercase">{s.label}</span>
          </div>
        ))}
      </div>
      ...
      <div className="flex items-center gap-2 text-[10px] font-mono">
        ...
        <span className="text-cyan-400">{edgeCount} edge</span>
        <span className="text-zinc-700">·</span>
